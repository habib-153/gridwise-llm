...
3. Runs that raw LLM output through a **deterministic guardrail validator** that enforces the exact schema, ranges, and safe-fallback rules, so the LLM's output is never trusted directly.
...
- **Library**: [`javascript-lp-solver`](https://github.com/JWally/jsLPSolver), a pure JS Simplex LP/MILP solver with no native dependencies.
